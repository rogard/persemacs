...
;; Make this into er317/lang-banner
(defun er317/latex-banner (text)
  "Insert a fixed-width 80-char LaTeX banner line."
  (interactive "sText: ")
  (let* ((prefix "% ── ")
         (content (concat text " "))
         (total-width 80)
         (fill (max 0 (- total-width (length prefix) (length content))))
         (line (concat prefix content (make-string fill ?─))))
    (insert line "\n")))

(defun er317/code-banner (text leader &optional depth)
...
(use-package lsp-mode
  ...
  ;; Add sh-mode hook with npm check and graceful fallback
  (add-hook 'sh-mode-hook
            (lambda ()
              (if (executable-find "npm")
                  (lsp-deferred)
                (message "npm not found — skipping LSP for sh-mode")))))
