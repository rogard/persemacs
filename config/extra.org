...
#+begin_src emacs-lisp
  (defun erw/json-safe (string &optional first &rest re-rep)
    "Make a string JSON-safe by replacing regex patterns with replacements from alist."
    (if first
        (if (null re-rep)
            string
          (let* ((pair (car re-rep))
            ...
            (apply 'erw/json-safe result t (cdr re-rep))))
      (apply 'erw/json-safe string t erw/json-safe-alist)))
#+end_src
