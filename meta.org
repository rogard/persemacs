...
#+begin_src emacs-lisp
  (let* ((plist (funcall er317/resolve-entry name))
...
#+begin_src emacs-lisp
  (let* ((plist (funcall er317/resolve-entry "init-source"))
         (basename (plist-get plist :basename)))
...
|-------------+------------------+------------------+-------------------------|
| init-source | "."              | "er317-init.el"   | Emacs init (source)     |
| init-target | "~"              | ".emacs"         | Emacs init (target)     |
| config      | "."              | "er317-config.el" | Packages' configuration |
| extra       | "."              | "er317-extra.el"  | Extra functionality     |
| link        | "."              | "er317-link.sh"   | target->source          |
...
#+RESULTS:
: #[(name) ((let ((entry (assoc name table))) (if entry nil (error "Unknown entry: %S" name)) (let* ((--cl-rest-- entry) (_name (if (= (length --cl-rest--) 4) (car-safe (prog1 --cl-rest-- (setq --cl-rest-- (cdr --cl-rest--)))) (signal 'wrong-number-of-arguments (list '(_name dir basename caption) (length --cl-rest--))))) (dir (car-safe (prog1 --cl-rest-- (setq --cl-rest-- (cdr --cl-rest--))))) (basename (car-safe (prog1 --cl-rest-- (setq --cl-rest-- (cdr --cl-rest--))))) (caption (car-safe --cl-rest--))) (let ((dir (string-remove-suffix "/" dir))) (list :name _name :directory dir :basename basename :path (expand-file-name basename dir) :caption caption))))) ((table ("init-source" "." "er317-init.el" "Emacs init (source)") ("init-target" "~" ".emacs" "Emacs init (target)") ("config" "." "er317-config.el" "Packages' configuration") ("extra" "." "er317-extra.el" "Extra functionality") ("link" "." "er317-link.sh" "target->source") ("org-capture" "~/src/org" "capture.org" "org capture target") ("trash" "~/.local/share" "Trash" "Trash")))]

Test:
#+header: :results drawer
#+begin_src emacs-lisp
  (funcall er317/resolve-entry "config")
#+end_src

#+RESULTS:
:results:
(:name config :directory . :basename er317-config.el :path /home/erwann/github/rogard/persemacs/er317-config.el :caption Packages' configuration)
:end:
...
#+RESULTS:
| /home/erwann/github/rogard/persemacs/er317-link.sh | /home/erwann/github/rogard/persemacs/er317-extra.el | /home/erwann/github/rogard/persemacs/er317-config.el | /home/erwann/github/rogard/persemacs/er317-init.el |

*** link

~ctrl-c ctrl-c~
#+header: :var path=(plist-get (funcall er317/resolve-entry "link") :path)
#+begin_src shell
  chmod +x "$path"
  $SHELL "$path"
#+end_src
...
*** init
:PROPERTIES:
:header-args: emacs-lisp :tangle (plist-get (funcall er317/resolve-entry "init-source") :path)
:END:
...
#+header: :var config-path=(plist-get (funcall er317/resolve-entry "config") :path)
#+header: :var extra-path=(plist-get (funcall er317/resolve-entry "extra") :path)
#+header: :noweb yes
#+begin_src emacs-lisp
...
#+header: :noweb yes
#+header: :var init-source=(plist-get (funcall er317/resolve-entry "init-source") :path)
#+begin_src emacs-lisp
  <<el-cleanup>>
#+end_src
...
*** config
:PROPERTIES:
:header-args: emacs-lisp :tangle (plist-get (funcall er317/resolve-entry "config") :path)
:END:
...
#+header: :var trash-path=(plist-get (funcall er317/resolve-entry "trash") :path)
#+begin_src emacs-lisp
  ;; :init
...
#+header: :var target-path=(plist-get (funcall er317/resolve-entry "org-capture") :path)
...
*** extra
:PROPERTIES:
:header-args: emacs-lisp :tangle (plist-get (funcall er317/resolve-entry "extra") :path)
:END:
...
#+begin_src emacs-lisp
  (er317/org-dependency--format-item "bar" "foo")
#+end_src
...
#+name: org-dependency
#+header: :var curr-cid=(er317/org-custom-id-current)
#+header: :var dep-cid-list='()
#+header: :tangle no :results drawer
#+begin_src emacs-lisp
  (let* ((items (mapcar (lambda (dep-cid)
                           (er317/org-dependency--format-item curr-cid dep-cid))
                         dep-cid-list))
         (item-block (if items
...
*** link
:PROPERTIES:
:header-args: emacs-lisp :tangle (plist-get (funcall er317/resolve-entry "link") :path)
:END:
...
#+header: :noweb yes
#+header: :var vc_path=(plist-get (funcall er317/resolve-entry "init-source") :path)
#+header: :var su_path=(plist-get (funcall er317/resolve-entry "init-target") :path)
#+begin_src shell
  <<sh-link>>
#+end_src
