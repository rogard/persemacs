...
#+header: :noweb-ref el-trash
#+begin_src emacs-lisp
  (cl-defun erw/org-refactor-helper-call (fn &rest keyval)
    "Call FN using `erw/org-keyval-conversion-map`."
    (let* ((entry (assoc fn erw/org-refactor-map))
           (input-fn (plist-get (cdr entry) :input))
           (output-fn (plist-get (cdr entry) :output))
           (fn-args (if (null input-fn)
                        nil
                      (funcall input-fn keyval)))
           (raw-output (apply fn fn-args)))
      (if (functionp output-fn)
          (funcall output-fn raw-output)
        (progn
          (warn "No valid output function found for %s in `erw/org-refactor-map`; returning raw output instead" fn)
          raw-output))))
#+end_src
...
#+begin_src emacs-lisp
  (cl-defmacro erw/org-refactor-helper-define (&key
  					     ...
  					     ((:target target-fn)))
    "Define TARGET-FN as a wrapper around `erw/org-refactor-helper-call`, forwarding to SOURCE-FN. INPUT-LAMBDA and OUTPUT-LAMBDA describe how to transform input/output keyvals."
    `(progn
       (add-to-list 'erw/org-refactor-map
                    (list :source ',source-fn
  			:target ',target-fn
  			:input ,input-lambda
  			:output ,output-lambda))
       (cl-defun ,target-fn (&rest keyval)
         ,(format "Wrapper around `%s` using `erw/org-refactor-helper-call`." source-fn)
       (apply #'erw/org-refactor-helper-call (function ,source-fn) keyval))))
#+end_src
  ...
  (cl-defmacro erw/org-refactor-helper-define (&key
    ...
    `(cl-defun ,target-fn (&rest keyval)
       ...
       (let* ((result-raw (if ,args-lambda
  			...
  			result-raw))))
         (message "debug: result-raw= %s" result-raw)
