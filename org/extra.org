...
#+begin_src emacs-lisp
  ...
  When provided, KEY-LIST is passed to ENCODE-FN."
    (let* ((raw-blocks (erw/noweb-ref-collect :head head :ref-list ref-list :tail tail))
           (parsed-values (mapcar parse-fn raw-blocks)))
      (funcall encode-fn
               (if key-list
                   (cl-pairlis key-list parsed-values)
                 parsed-values))))
#+end_src
...
#+begin_src emacs-lisp
    (let* ((head "^json-")(list
  (erw/noweb-ref-collect :head head :ref-list '("foo" "bar"))))
...
#+begin_src emacs-lisp
    (let* ((head "^json-"))
      (erw/noweb-ref-encode :head head :ref-list '("foo" "bar") :parse-fn 'json-read-from-string :encode-fn 'json-encode-array))
#+end_src
...
#+begin_src emacs-lisp
    (let* ((head "^json-"))
      (erw/noweb-ref-encode :head head :ref-list '("foo" "bar") :key-list '("foo" "bar") :parse-fn 'json-read-from-string :encode-fn 'json-encode))
#+end_src
