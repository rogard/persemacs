...
#+begin_src emacs-lisp
(cl-defun erw/noweb-expand-ref (regex)
  "Expands all source blocks whose :noweb-ref matches REGEX and returns their expanded contents concatenated as a string."
  (let ((results '()))
...
#+header: :noweb-ref el-noweb
#+begin_src emacs-lisp
  (cl-defun erw/noweb-expand-ref-collect (&key head tail references)
    "Builds a regex from REFERENCES with optional HEAD and TAIL anchors, then forwards to `erw/noweb-expand-ref`."
    (let* ((regex-body (mapconcat #'identity references "\\|"))
           (regex (concat
                   head "\\(" regex-body "\\)" tail)))
      (erw/noweb-expand-ref regex)))
#+end_src
...
#+header: :noweb yes
#+begin_src emacs-lisp
(erw/noweb-expand-ref "sh-foo\\|bar")
#+end_src
...
#+begin_src emacs-lisp
  (erw/noweb-expand-ref-collect :head "sh-" :references '("foo" "bar"))
#+end_src
