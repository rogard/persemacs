...
#+begin_src emacs-lisp
  (cl-defmacro er317/exception-raise (&key type (message "ARGS: %S") (arg-list nil) (type-if-nil (if type type 'error)))
...
#+header: :noweb-ref el-mangle-meta
#+begin_src emacs-lisp
  ...
  (defun er317/read-defined-symbols
      (&key
       ...
       (string (when noweb-ref-plist
                 (apply #'er317/noweb-ref-expand noweb-ref-plist)))
       file
       (buffer (cond
...
#+header: :noweb-ref el-func
#+begin_src emacs-lisp
  (cl-defmacro er317/func-refactor
      (&key
       ...
       (arg-list-func
        `(lambda (&rest kv)
      	 (mapcar (lambda (k) (plist-get kv k))
      		 ,target-key-list))))
    (condition-case err
        (let ((target-symb
               (cond
          ...
          `(cl-defun ,target-symb (&rest keyval) ,(format "Refactored wrapper around `%s`, accepting keyword args %s." source-func target-key-list)
                     (condition-case err
                         (let* ((result-raw
                                 (let ((arg-list (apply ,arg-list-func keyval)))
  				 (apply ,source-func arg-list)))
                                (result
                                 (if ,result-func
                                     (funcall ,result-func result-raw)
  				 
  				 (progn
                                     (warn "No output function found for %s; returning raw output"
  					 ',target-symb)
                                     result-raw))))
  			 result)
                       (error
...
#+begin_src emacs-lisp  
  (er317/func-refactor
   ...
   :target-key-list '(:x :y)
   :result-func (lambda (result) (* result 10))
...
#+begin_src emacs-lisp
  (er317/func-refactor
   ...
   :target-func 'er317/org-babel-info
   :key-target-list '(:no-eval :datum)
...
#+begin_src emacs-lisp
  (defvar er317/noweb-ref-keyval-alist nil "Key-value list for noweb-ref")
#+end_src

#+RESULTS:
: er317/noweb-ref-keyval-alist

#+header: :noweb-ref el-noweb-ref
#+begin_src emacs-lisp
  (cl-defun er317/noweb-ref-put-backend (&key group method ((:value plist)))
    "Store PLIST under (GROUP . METHOD)."
    (let ((key (cons group method))
  	(existing (assoc (cons group method) er317/noweb-ref-keyval-alist)))
      (if existing
  	(setcdr existing plist)
        (push (cons key plist) er317/noweb-ref-keyval-alist))))
#+end_src

#+RESULTS:
: er317/noweb-ref-put-backend

#+header: :noweb-ref el-example
#+begin_src emacs-lisp
  (er317/noweb-ref-put-backend :group "test"  :method "foo" :value '(:func (lambda () (message "hello"))))
#+end_src
...
#+header: :noweb-ref el-noweb-ref
#+begin_src emacs-lisp
(cl-defun er317/noweb-ref-get-backend (&key group method key)
...
ARGS must contain exactly one key from the known keys."
  (let* ((lookup-key (cons group method))
         (prop-list (cdr (assoc lookup-key er317/noweb-ref-keyval-alist))))
    (plist-get prop-list key)))
#+end_src

#+RESULTS:
: er317/noweb-ref-get-backend

#+header: :noweb-ref el-example
#+begin_src emacs-lisp
  (er317/noweb-ref-get-backend :group "test" :method "foo" :key :func)
#+end_src
...
#+header: :noweb-ref el-noweb-ref
#+begin_src emacs-lisp
  (cl-defmacro er317/noweb-ref-keyval-define (&key group msg-type (symb-list '(func doc)))
          ...
          `(progn
             ...
             (cl-defun ,get-name (&rest rest)
               ,(format "Frontend get for group `%s`, forwarding keys %s to backend"
                        group-name (mapconcat #'symbol-name key-list ", "))
               (apply #'er317/noweb-ref-get-backend
...
#+header: :noweb-ref el-noweb-ref
#+begin_src emacs-lisp
  (er317/noweb-ref-keyval-define :group 'helper)
#+end_src

#+RESULTS:
: er317/noweb-ref-helper-get

#+header: :noweb-ref el-noweb-ref
#+header: :results raw
#+begin_src emacs-lisp
  (er317/noweb-ref-helper-put
   :method 'buffer-re
   :func '(lambda (ref-list)
  	 (format "\\(%s\\)" (string-join ref-list "\\|")))
   :doc "Combines a ref-list into a non-anchored regex suitable to parse a buffer")
#+end_src

#+RESULTS:
(:func (lambda (ref-list) (format \(%s\) (string-join ref-list \|))) :doc Combines a ref-list into a non-anchored regex suitable to parse a buffer)

#+begin_src emacs-lisp
  (pp er317/noweb-ref-keyval-alist)
#+end_src

#+RESULTS:
: (((helper . buffer-re) :func
:   (lambda (ref-list) (format "\\(%s\\)" (string-join ref-list "\\|")))
:   :doc
:   "Combines a ref-list into a non-anchored regex suitable to parse a buffer")
:  (("test" . "foo") :func (lambda nil (message "hello"))))

#+header: :noweb-ref el-example
#+begin_src emacs-lisp
 (funcall (er317/noweb-ref-get-backend :group 'helper :method 'buffer-re :key :func) '("foo" "bar"))
#+end_src
...
#+begin_src emacs-lisp
  (cl-defun er317/noweb-ref-filter
      (&key
       ref-list
       (head-re "^")
       (middle-re (string-join ref-list "\\|"))
       (tail-re "$")
       (regex (concat head-re (format "\\(%s\\)" middle-re) tail-re))
       func-key
       (func
        (cond (func-key (er317/noweb-ref-keyval-get :info-func func-key))
              (t #'identity)))
       arg-list)
    "Filters source block info by matching noweb-ref on REGEX, transforming each using FUNC+ARG-LIST."
    (let ((results '()))
...
#+begin_src emacs-lisp
  (cl-defun er317/noweb-ref-fold
      (&key
       ...
       (fold-func (
         (cond
          (fold-func (er317/nowef-ref-fold-func fold-func-key))
          (fold-func fold-func)
          (t (er317/exception-raise :message "Missing FOLD-FUNC")))))
        
        "Assemble source block info using noweb-refences for collection.
