...
#+begin_src emacs-lisp
(cl-defmacro er317/func-refactor (&key
  ...
  (condition-case err
      (let ((args-plist (list
                         ...
                         :msg-type msg-type
                         :target-key-list target-key-list)))
        (cond
         ;; Auto-generate args-func if not provided and target-key-list is available
         ((not args-func)
          (if (not target-key-list)
              (error "Cannot determine args-func — supply :args-func or :target-key-list")
            `(er317/func-refactor
              ,@(plist-put args-plist
                           :args-func
                           `(lambda (&rest kv)
                              (mapcar (lambda (k) (plist-get kv k))
                                      ',target-key-list))))))

         ;; Handle quoted symbol for :target-func (e.g. 'my-func)
         ((not (symbolp target-func))
          (if (and (consp target-func) (eq (car target-func) 'quote))
              `(er317/func-refactor
                ,@(plist-put args-plist :target-func (cadr target-func)))
            (error "target-func must be a symbol or quoted symbol")))

         ;; Final case: All arguments resolved — emit function definition
         (t
          `(cl-defun ,target-func (&rest keyval)
             ,(format "Refactored wrapper around `%s`, accepting keyword args %s."
                      source-func target-key-list)
             (condition-case err
                 (let* ((result-raw
                         (let ((args-list (apply ,args-func keyval)))
                           (apply ,source-func args-list)))
                        (result
                         (if ,result-func
...
#+begin_src emacs-lisp
  (er317/func-refactor
   ...
   :args-func 'identity
;;   :msg-type msg-type
   :result-func (lambda (info)
  (let ((keyval-list '(:lang 0 :body 1 :header-args 2 :switches 3 :name 4 :start 5 :coderef 6))
        (result '()))
    (progn
      (unless (and info (listp info) (>= (length info) 3)) ;; <-- syntax highlighting bug
        (error "Invalid Org Babel info object %s" info))
             (while keyval-list
               (let ((key (pop keyval-list))
                     (pos (pop keyval-list)))
                 (push key result)
                 (push (nth pos info) result)))
             (nreverse result)))))) ;; <-- one more highlighter due to bug
#+end_src
...
#+begin_src emacs-lisp
    ;;  (cl-defun er317/org-babel-info-as-plist (&key info)
