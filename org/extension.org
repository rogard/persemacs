...
#+begin_src emacs-lisp
    (defvar er317/noweb-ref-keyval-alist nil
      "Key-value list for noweb-ref")
#+end_src
...
:   #[(ref-list) ((format "\\(%s\\)" (string-join ref-list "\\|"))) nil]
:   :doc
...
#+header: :noweb-ref el-noweb-ref
#+begin_src emacs-lisp
  (defun er317/noweb-ref-put-backend (group method plist)
...
#+header: :noweb-ref el-noweb-ref
#+begin_src emacs-lisp
  (defun er317/noweb-ref-get-backend (group method &key &rest args)
  ...
  ARGS must contain exactly one key from the known keys."
    (let* ((lookup-key (cons group method))
           ...
           (key-list (cl-loop for k on args by #'cddr collect (car k)))
           (requested
            (cl-loop for k in key-list
                     when (plist-member args k)
                     collect k)))
      (unless (= (length requested) 1)
        (error "Expected exactly one keyword key in get: %S" requested))
      (plist-get prop-list (car requested))))
#+end_src
  ...
  (cl-defmacro er317/noweb-ref-keyval-define (&key group msg-type (symb-list '(func doc)))
    ...
    (condition-case err
        (let* ((group-symb (if (and (consp group) (eq (car group) 'quote))
               ...
               (key-list (cl-loop for symb in symb-list
                                  collect (intern (format ":%s" (symbol-name symb)))))
               (put-name (intern (format "er317/noweb-ref-%s-put" group-name)))
               (get-name (intern (format "er317/noweb-ref-%s-get" group-name))))
          `(progn
             ;; Frontend PUT forwards plist to backend
             ...
             ;; Frontend GET forwards args to backend
             (cl-defun ,get-name (&rest rest)
               ,(format "Frontend get for group `%s`, forwarding keys %s to backend"
                        group-name (mapconcat #'symbol-name key-list ", "))
               (let ((valid-keys ',key-list)
                     (filtered (cl-loop for (k v) on rest by #'cddr
                                        when (and (memq k valid-keys) v)
                                        collect k)))
                 (unless (= (length filtered) 1)
                   (error "Expected exactly one valid key from %S; got %S" valid-keys filtered))
                 (apply #'er317/noweb-ref-get-backend
                        ',group-symb
                        (plist-get rest :method)
                        rest)))))
      (er317/exception-raise
       :type msg-type
       :message (error-message-string err))))
...
#+RESULTS:
| :func | #[(ref-list) ((format \(%s\) (string-join ref-list \ | ))) nil] | :doc | Combines a ref-list into a non-anchored regex suitable to parse a buffer |

#+header: :noweb-ref el-example
#+header: :results raw code
#+begin_src emacs-lisp 
  (funcall 
   (er317/noweb-ref-helper-get :method 'buffer-re :key :func) '("x" "y"))
#+end_src
