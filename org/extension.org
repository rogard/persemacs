...
#+begin_src emacs-lisp :wrap src json
  (let* ((info (org-babel-find-named-block)))
    (funcall (er317/group-info-get :method 'body :func t) info))
#+end_src
...
#+begin_src emacs-lisp
  (cl-defun er317/noweb-ref-filter
      (&key
       ref-list
       (head-re "^")
       (middle-re (funcall (er317/regex-get :method union-re :key :func) ref-list))
       (tail-re "$")
       (regex (concat head-re (format "\\(%s\\)" middle-re) tail-re))
       (info-symb 'body)
       (func (er317/group-babel-info :method info-symb :key :func))
       arg-list)
      ...
      (&key
       ...
       (fold-func (
         (cond
          ...
          (fold-func fold-func)
          (t (er317/raise :message "Missing FOLD-FUNC")))))
