...
#+begin_src emacs-lisp
  (cl-defun er317/symb-combo-next (&key (start-key nil) (next-symb nil) (msg-type 'error) (key-p t))
    "Return a new interned symbol by appending NEXT-SYMB to START-KEY using a separator."
    (condition-case err
        (cond
         ...
         ((and key-p (not (keywordp start-key)))
...
#+header: :noweb-ref el-example
#+begin_src emacs-lisp
  (er317/symb-combo-next :start-key ':foo :next-symb 'bar)
#+end_src
...
#+header: :noweb-ref el-symb
#+begin_src emacs-lisp
  (cl-defun er317/symb-combo-list (&key (start-key nil) (symb-list '()) (msg-type 'error) (key-p t))
    ...
    (condition-case err
  	...
  	(error "SYMB-LIST is not a list: %S" symb-list))
         ...
         ((null symb-list)
  	(er317/symb-combo-next :start-key start-key :key-p key-p))
         (t
...
#+begin_src emacs-lisp
  (er317/symb-combo-list :start-key :foo :symb-list '(bar))
#+end_src
