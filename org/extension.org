...
#+begin_src emacs-lisp
(cl-defmacro er317/exception-raise (&key (type 'error) (message "ARGS: %S") (arg-list nil))
...
#+begin_src emacs-lisp
  (defvar er317/noweb-ref-keyval-alist nil "Backend")
#+end_src
...
#+header: :noweb-ref el-noweb-ref
#+begin_src emacs-lisp
  (cl-defmacro er317/noweb-ref-keyval-define (&key group)
    "Define put/get functions for GROUP using an alist with plist values."
    (let* ((group-symb (if (and (consp group) (eq (car group) 'quote))
                           (cadr group)
                         group))
           (group-name (symbol-name group-symb))
           (put-fun-name (intern (format "er317/noweb-ref-%s-put" group-name)))
           (get-fun-name (intern (format "er317/noweb-ref-%s-get" group-name))))
      `(progn
         ;; PUT FUNCTION
         (cl-defun ,put-fun-name (&key method func doc)
           ,(format "Associate FUNC and DOC with (%s . METHOD) in `er317/noweb-ref-keyval-alist`." group-name)
           (let* ((key (cons ',group-symb method))
                  (existing (assoc key er317/noweb-ref-keyval-alist))
                  (value (list :func func :doc doc)))
             (if existing
                 (setcdr existing value)
               (push (cons key value) er317/noweb-ref-keyval-alist))))

         ;; GET FUNCTION
         (cl-defun ,get-fun-name (&key method key)
           ,(format "Access value for KEY (:func or :doc) from (%s . METHOD) in `er317/noweb-ref-keyval-alist`." group-name)
           (let* ((lookup-key (cons ',group-symb method))
                  (prop-list (cdr (assoc lookup-key er317/noweb-ref-keyval-alist))))
             (message "DEBUG GET [%s . %s] => %S" ',group-symb method prop-list)
...
#+begin_src emacs-lisp :results value
  (er317/noweb-ref-helper-get :method 'buffer-re :key :func)
#+end_src

#+RESULTS:

#+begin_src emacs-lisp
  (makunbound 'er317/debug-plist)
  (defvar er317/debug-plist nil)
#+end_src

#+begin_src emacs-lisp
  (setq er317/debug-plist (plist-put er317/debug-plist (cons 'x 'y) 'z))
#+end_src
...
#+begin_src emacs-lisp
  (plist-get er317/debug-plist (cons 'x 'y))
#+end_src
...
#+begin_src emacs-lisp
  (cl-defun er317/func-default (&key func key)
