...
#+begin_src emacs-lisp
  (defconst er317/noweb-ref-key-main-list "Main key list for `noweb-ref`"
    '(ref-list
      (head-re "^")
...
#+begin_src emacs-lisp
  (cl-defmacro er317/noweb-ref-define
      (&key (prefix "er317/noweb-ref")
            suffix-name
            (doc-string "A `noweb-ref` function")
            (key-main-list er317/noweb-ref-key-main-list)
            key-other-list
            body)
    `(cl-defun ,(intern (concat prefix suffix-name))
         (&key ,@key-main-list ,@key-other-list)
       ,doc-string
       ,@body))
#+end_src
...
#+begin_src emacs-lisp
  (er317/noweb-ref-define
   :suffix-name "info-list"
   :doc-string "Collect source block info whose :noweb-ref matches REGEX."
   :body (let ((results '()))
...
#+begin_src emacs-lisp
  (er317/noweb-ref-define
   :suffix-name "expand"
   :doc-string "Expand noweb REF-LIST, each surrounded by HEAD and TAIL.
    ...
    (condition-case err
      ...
      (error
       (er317/raise
        :type msg-type
        :message (error-message-string err)
        :arg-list (list :start-symb start-symb :symb-list symb-list)))))
#+end_src
...
#+begin_src emacs-lisp
  (cl-defmacro
      er317/generic-keyval-put (&key symb-list value)
    "Associate VALUE with KEY in `er317/generic-keyval-plist`.

  Arguments:
    :symb-list — A symbol or keyword identifying the handler.
...
#+begin_src emacs-lisp
  (defun er317/generic-keyval-get (&key symb-list)
    "Return value for KEY"
    (plist-get er317/generic-keyval-plist symb-list))
#+end_src
