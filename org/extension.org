...
#+header: :noweb-ref symbol
#+begin_src emacs-lisp
  (cl-defun er317/stamp-replace
      version-url ;; example: https://raw.githubusercontent.com/rogard/persemacs/77b3f0c08ae5672a9dad27513e41edaefe7d2955/el/noweb.el
      (symbol-list )
    (author-alias "er317")
    sha-full
    (sha-short (substring))
  ...
  ""
  (cond
   (null symbol-list) result)
  (let* (
         (current-name (symbol-name (car symbol-list)))
         (new-name (format format-control ))
         ))
  )
#+end_src

#+begin_src emacs-lisp
  (cl-defun er317/symbol-versionize (&key
  				   (author-alias "er317")
  				   sha-full
  				   symbol-list
  				   sha-short (substring shall-full 0 7)
  				   regex (format "^%s/%s" author-alias "[^ ]+")
  				   replace (format "%s-%s/%s" author-alias sha-short "\1")
  				   "")
#+end_src
...
#+begin_src emacs-lisp
       (cl-defun er317/inspect (&key

       			 (file
(let ((make-temp-file prefix dir-flag suffix)
